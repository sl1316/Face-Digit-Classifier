...
class NaiveBayesClassifier(classificationMethod.ClassificationMethod):
  ...
  def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, kgrid):
    ...
    oPrior = util.Counter()
    oConditionalProb = util.Counter()
    oCounts = util.Counter()
    ...
    for k in kgrid:
        ...
        # get counts from ordinary training step
        for key, val in oPrior.items():
            prior[key] += val
        for key, val in oCounts.items():
            counts[key] += val
        for key, val in oConditionalProb.items():
